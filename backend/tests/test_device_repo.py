...
def test_return_and_repair_scrap(db):
    d = _dev(db, "AST-2")
    e = _emp(db, "E2")
    device_repo.checkout_if_available(db, d.id, e.id, date.today(), d.version, "op")
    db.flush()
    assert device_repo.return_if_in_use(db, d.id, "op") == 1
    db.flush()
    assert device_repo.to_repair(db, d.id, "op") == 1
    db.flush()  # in-stock -> repair
    db.refresh(d)
    assert d.status == DeviceStatus.REPAIR
